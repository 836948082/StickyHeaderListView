...
public class MainActivity extends AppCompatActivity implements SmoothListView.ISmoothListViewListener {
    ...
    private static final int HEIGHT = 95;// 标题栏高度 ＋ FilterView的高度
    private static final int PADDING = 20; //ListView的item的paddingTop和paddingBottom的值
    ...
    private void fillAdapter(List<TravelingEntity> list) {
        if (list == null || list.size() == 0) {
            ...
            int height = mScreenHeight - DensityUtil.dip2px(mContext, HEIGHT); // 95 = 标题栏高度 ＋ FilterView的高度
            mAdapter.setData(ModelUtil.getNoDataEntity(height));
        } else {
            int child_height = smoothListView.getChildAt(0).getHeight() + DensityUtil.dip2px(mContext, PADDING);
            int listview_height = smoothListView.getHeight();
            TravelingAdapter.ONE_SCREEN_COUNT = listview_height / child_height + 1;
    ...
    public void setListViewHeightBasedOnChildren(ListView listView) {
        ...
        for (int i = 0; i < listAdapter.getCount(); i++) {
            Log.e("params.height", "params.height++" + listAdapter.getCount());
